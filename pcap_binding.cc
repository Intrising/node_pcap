...
public:
  ...
  static void Init(Handle<Object> target)
  {
    ...
    NODE_SET_PROTOTYPE_METHOD(s_ct, "inject", Inject);
  ...
  static Handle<Value> Inject(const Arguments& args)
{
    HandleScope scope;
    ...
    if (!Buffer::HasInstance(args[0])) {
	  ...
	  nb =pcap_inject(sess->pcap_handle, sess->buffer_data, sess->buffer_length);
    return scope.Close(Integer::NewFromUnsigned(nb));
}
...
static Handle<Value> Open(bool live, const Arguments& args)
{
    ...
    if (live) {
        ...
        if (pcap_set_promisc(sess->pcap_handle, 1) != 0) {
            return ThrowException(Exception::Error(String::New("error setting promiscuous mode")));
        }
	/*	
        if (pcap_setdirection(sess->pcap_handle, PCAP_D_IN) != 0) {
            return ThrowException(Exception::Error(String::New("error setting capture direction")));
        }*/
        ...
        sess->pcap_dump_handle = NULL;
        if (strlen((char *) *pcap_output_filename) > 0) {
            sess->pcap_dump_handle = pcap_dump_open(sess->pcap_handle, (char *) *pcap_output_filename);
            if (sess->pcap_dump_handle == NULL) {
...
//
  static void PacketReady(u_char *callback_p, const struct pcap_pkthdr* pkthdr, const u_char* packet) {
    ...
    if (sess->pcap_dump_handle != NULL) {
        pcap_dump((u_char *) sess->pcap_dump_handle, pkthdr, packet);
    }
  ...
  static Handle<Value> Close(const Arguments& args)
{
    ...
    if (sess->pcap_dump_handle != NULL) {
        pcap_dump_close(sess->pcap_dump_handle);
        sess->pcap_dump_handle = NULL;
    }
...
extern "C" void init (Handle<Object> target)
{
    ...
    //target->Set(String::New("inject"), FunctionTemplate::New(Inject)->GetFunction());
